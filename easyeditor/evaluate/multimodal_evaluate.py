...
def compute_icl_multimodal_edit_quality(
    ...
    if "fg_gen_q" in record.keys():
        fg_gen_acc, _ = icl_multimodal_lm_eval(
            model,
            model_name,
            hparams,
            tok,
            icl_examples,
            record["fg_gen_a"],
            f'New Fact: {prompt} {target}\nPrompt: {record["fg_gen_q"]}',
            image,
        )
        ret["fg_gen_acc"] = fg_gen_acc
    ...
    if "fg_loc_q" in record.keys():
        if pre_edit:
            _, _, fg_loc_output = icl_multimodal_lm_eval(
                model,
                model_name,
                hparams,
                tok,
            ...
            _, _, fg_loc_output = icl_multimodal_lm_eval(
                ...
                icl_examples,
                record["fg_loc_a"],
                f'New Fact: {prompt} {target}\nPrompt: {record["fg_loc_q"]}',
                image,
                is_loc=True,
            )
        ret["fg_loc_output"] = fg_loc_output

    return ret


def compute_fine_grained_edit_quality(
        model,
        model_name,
        hparams: HyperParams,
        tok: AutoTokenizer,
    ...
    rephrase_root = hparams.rephrase_image
    # First, unpack rewrite evaluation record.
    # dict_keys(['prompt', 'target', 'image', 'cond', 'fine_grained_locality_prompt', 'fine_grained_locality_ground_truth'])
    target = record["target"]
    prompt = record["prompt"]
    image = record["image"] if record["image"].is_cuda else record["image"].to(hparams.device)

    if "fine_grained_locality_prompt" in record.keys():
        loc_q = record["fine_grained_locality_prompt"]
        loc_a = record["fine_grained_locality_ground_truth"]
    ...
    if "fine_grained_locality_prompt" in record.keys():
        if pre_edit:
            _, _, fine_grained_locality_output = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,
                                                                 loc_a, loc_q, image, is_loc=True)
        else:
            _, _, fine_grained_locality_output = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,
                                                                 loc_a, f'New Fact: {prompt} {target}\nPrompt: {loc_q}', image, is_loc=True)
        ret['fine_grained_locality_output'] = fine_grained_locality_output
...
def pred_edit_text(
    ...
    pred_text,targ_text,acc = get_pred_text(model, model_name, hparams, tok, icl_examples,
                                             target, prompt, image)
...
def get_pred_text(
    ...
    filtered_pred = pred_ids[pred_ids != 0].view(1,-1)
    pred_text = [tokenizer.decode(ids,skip_special_tokens=True) for ids in filtered_pred]
    filtered_targ = targ[targ != 0].view(1,-1)
    targ_text = [tokenizer.decode(ids,skip_special_tokens=True) for ids in filtered_targ]

    pred_cpu = pred_ids.detach().cpu()
    targ_cpu = targ.detach().cpu()
    correct = pred_cpu == targ_cpu
    if logits.dim() == 3:
        correct = (pred_ids == targ).all(-1)  # We aim for an exact match across the entire sequence
    acc = correct.float().mean()

    return pred_text,targ_text,acc
