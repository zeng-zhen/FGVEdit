...
class LocTrainer(BaseTrainer):
    ...
    def edit_step(self, batch, training: bool):
        ...
        with torch.no_grad():
            ...
            base_fg_outputs = self.model(batch["fg_loc"])
            if not isinstance(base_fg_outputs, torch.Tensor):
                base_fg_logits = base_fg_outputs.logits
            else:
                base_fg_logits = base_fg_outputs
        ...
        with torch.set_grad_enabled(training):
            ...
            # rephrase fine_grained
            post_fg_edit_outputs = edited_model(batch["fg_gen"])
            if not isinstance(post_fg_edit_outputs, torch.Tensor):
                post_fg_edit_logits = post_fg_edit_outputs.logits
                post_fg_batch_labels = post_fg_edit_outputs.labels
            else:
                post_fg_edit_logits = post_fg_edit_outputs
                post_fg_batch_labels = batch["fg_gen"]["labels"]
            ...
            l_fg_edit = self.model.edit_loss_fn(self.config, post_fg_edit_logits, post_fg_batch_labels, multimodal=True)["nll"] 


            # Collect some useful metrics  
            # post_edit_dict:           T-Generality
            # inner_edit_dict:          Reliability
            # image_rephrase_edit_dict: M-Generality
            with torch.no_grad():
                ...
                fg_gen_edit_dict = self.model.edit_loss_fn(self.config, post_fg_edit_logits, post_fg_batch_labels, multimodal=True)
            ...
            post_fg_base_outputs = edited_model(batch["fg_loc"])
            if not isinstance(post_fg_base_outputs, torch.Tensor):
                post_fg_base_logits = post_fg_base_outputs.logits
                kl_fg_mask = post_fg_base_outputs.attention_mask
            else:
                post_fg_base_logits = post_fg_base_outputs
                kl_fg_mask = torch.ones(post_fg_base_logits.shape[0], post_fg_base_logits.shape[1]).to(base_fg_logits.device)
            ...
            l_fg_loc = kl_loc_loss(base_fg_logits.detach(), post_fg_base_logits, mask=kl_fg_mask)
        ...
        # l_edit:       T-Generality
        # l_loc:        T-locality
        # l_image_loc:  M-locality, not in SERAC_MULTI
        # l_image_edit: M-Generality

        if self.config.alg == "SERAC":
            l_total_edit = self.config.cedit * l_edit + self.config.cloc * l_loc + self.config.iedit * l_fg_edit
        else:
            l_total_edit = self.config.cedit * l_edit + self.config.cloc * (l_loc + l_fg_loc) + self.config.iedit * l_fg_edit
        ...
        # FG locality
        post_fg_base_logits_softmax_top_k = torch.topk(torch.nn.functional.softmax(post_fg_base_logits, dim=-1), k=10, dim=-1).indices
        base_fg_logits_softmax_top_k = torch.topk(torch.nn.functional.softmax(base_fg_logits, dim=-1), k=10, dim=-1).indices
        ...
        # M-Generality
        info_dict['fg_gen_image/acc'] = image_rephrase_edit_dict["acc"].item()
        ...
        # M-Locality
        info_dict["fg_image_loc/acc"] = sum(post_image_base_logits_softmax_top_k.view(-1) == base_image_logits_softmax_top_k.view(-1))/post_image_base_logits_softmax_top_k.view(-1).shape[0]
        # FG-Locality
        info_dict["fg_loc/acc"] = sum(post_fg_base_logits_softmax_top_k.view(-1) == base_fg_logits_softmax_top_k.view(-1))/post_fg_base_logits_softmax_top_k.view(-1).shape[0]
        info_dict['fg_gen/acc'] = fg_gen_edit_dict["acc"].item()
    ...
    def _inline_validation_log(self, step, stats, start_time, steps):
        ...
        image_acc = f"{stats['fg_gen_image/acc_val']:<12.5f}"
        loc_acc = f"{stats['loc/acc_val']:<12.5f}"
        loc_image_acc = f"{stats['fg_image_loc/acc_val']:<12.5f}"
        loc_fg_acc = f"{stats['fg_loc/acc_val']:<12.5f}"
        gen_fg_acc = f"{stats['fg_gen/acc_val']:<12.5f}"

        LOG.info(
          f"Step {prog} outer_acc: {outer_acc} image_acc: {image_acc} inner_acc: {inner_acc} it_time: {elapsed:.4f} loc_acc: {loc_acc}, image_loc: {loc_image_acc}, fg_loc: {loc_fg_acc}, fg_gen: {gen_fg_acc}"
        )
