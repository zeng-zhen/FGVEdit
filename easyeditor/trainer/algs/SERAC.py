    ...
    def __init__(self, model, config, model_constructor, classifier=None, classifier_tok=None,
        ...
        if replacement is None:
            if config.model_name == "minigpt4":
                ...
                self.replacement_tok = transformers.AutoTokenizer.from_pretrained(config.small_name, trust_remote_code=True, pad_token='<|endoftext|>', cache_dir='./hugging_cache')
            elif config.model_name == "owl-2":
                self.replacement_tok = transformers.AutoTokenizer.from_pretrained(config.small_name, trust_remote_code=True, use_fast=False, cache_dir='./hugging_cache')
            else:
            ...
            if self.config.freeze_cntr:
                ...
                if config.model_class == "BertClassifier":
                    ...
                    self.replacement = AutoModelForCausalLM.from_pretrained(config.small_name, trust_remote_code=True, cache_dir='./hugging_cache')
                    for k, v in self.replacement.named_parameters():
                        if '31' in k:
                            v.requires_grad = True
                        else:
                    ...
                    self.replacement = transformers.AutoModelForCausalLM.from_pretrained(config.small_name, trust_remote_code=True, cache_dir='./hugging_cache')
                    for k, v in self.replacement.named_parameters():
                        if '31' in k:
                            v.requires_grad = True
                        else:
        ...
        if config.model_name == 'qwen-vl':
            self.qwenvl_tok = transformers.AutoTokenizer.from_pretrained(config.name, trust_remote_code=True, pad_token='<|endoftext|>', cache_dir='./hugging_cache')
