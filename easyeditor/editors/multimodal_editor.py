...
from ..evaluate import (compute_icl_multimodal_edit_quality, 
                        pred_edit_text,
                        compute_fine_grained_edit_quality)
...
class MultimodalEditor:
    ...
    def __init__(self,
        ...
        self.apply_algo = ALG_MULTIMODAL_DICT[hparams.alg_name]  # apply_ike_to_multimodal_model
    ...
    def edit(self,
        ...
        for i, request in enumerate(requests):
            ...
            if 'fg_loc_output' in metrics['post'].keys():
                assert len(metrics['post']['fg_loc_output']) == len(metrics['pre']['fg_loc_output'])
                base_fg_logits = metrics['pre']['fg_loc_output'].to(torch.float32)
                post_fg_logits = metrics['post']['fg_loc_output'].to(torch.float32)
                if post_fg_logits.shape[1] > base_fg_logits.shape[1]:
                    post_fg_logits = post_fg_logits[:, -base_fg_logits.shape[1]:, :]
                else:
                    base_fg_logits = base_fg_logits[:, -post_fg_logits.shape[1]:, :]

                base_fg_top_k = torch.topk(torch.nn.functional.softmax(base_fg_logits, dim=-1), k=10, dim=-1).indices
                post_fg_top_k = torch.topk(torch.nn.functional.softmax(post_fg_logits, dim=-1), k=10, dim=-1).indices
                metrics['post']['fg_loc_acc'] = sum(post_fg_top_k.view(-1) == base_fg_top_k.view(-1)) / post_fg_top_k.view(-1).shape[0]
                metrics['post'].pop('fg_loc_output')
                metrics['pre'].pop('fg_loc_output')

            if 'fg_gen_acc' in metrics['post'].keys() and 'fg_loc_acc' in metrics['post'].keys():
                metrics['post']['specificity_acc'] = (metrics['post']['fg_gen_acc'] + metrics['post']['fg_loc_acc']) / 2
    ...
    def pre_edit(self,
        ...
        for i, request in enumerate(tqdm(ds, desc='Editing dataset', total=len(ds))):
            ...
            metrics = {
                'case_id': i,
                "pre": pred_edit_text(self.model, self.model_name, self.hparams, self.tok, [''],
                                                    request, self.hparams.device, pre_edit=True)
            }
    ...
    def edit_fine_grained_dataset(self,
                     ds: Dataset,
                     keep_original_weight=False,
                     verbose=True,
                     **kwargs
        ...
        for i, request in enumerate(tqdm(ds, desc='Editing dataset', total=len(ds))):
            ...
            metrics = {
                'case_id': i,
                "time": exec_time,
                "post": compute_fine_grained_edit_quality(self.model, self.model_name, self.hparams, self.tok, icl_examples,
                                                    request, self.hparams.device),
                "pre": compute_fine_grained_edit_quality(self.model, self.model_name, self.hparams, self.tok, [''],
                                                    request, self.hparams.device, pre_edit=True)
            }
            if 'fine_grained_locality_output' in metrics['post'].keys():
                assert len(metrics['post']['fine_grained_locality_output']) == \
                        len(metrics['pre']['fine_grained_locality_output'])
                base_image_logits = metrics['pre']['fine_grained_locality_output'].to(torch.float32)
                post_image_logits = metrics['post']['fine_grained_locality_output'].to(torch.float32)
                if post_image_logits.shape[1] > base_image_logits.shape[1]:
                    post_image_logits = post_image_logits[:, -base_image_logits.shape[1]:, :]
                else:
                    base_image_logits = base_image_logits[:, -post_image_logits.shape[1]:, :]
                ...
                base_image_logits_softmax_top_k = torch.topk(torch.nn.functional.softmax(base_image_logits, dim=-1), k=10, dim=-1).indices
                post_image_base_logits_softmax_top_k = torch.topk(torch.nn.functional.softmax(post_image_logits, dim=-1), k=10, dim=-1).indices
                metrics['post']['fine_grained_locality_acc'] = sum(post_image_base_logits_softmax_top_k.view(-1) == base_image_logits_softmax_top_k.view(-1))/post_image_base_logits_softmax_top_k.view(-1).shape[0]
                metrics['post'].pop('fine_grained_locality_output')
                metrics['pre'].pop('fine_grained_locality_output')
